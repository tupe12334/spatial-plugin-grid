...
Versioning and changelogs are owned exclusively by [Changesets](https://github.com/changesets/changesets); commit messages are enforced as [Conventional Commits](https://www.conventionalcommits.org/) by commitlint (`commitlint.config.js`, `.husky/commit-msg`, and the `Commitlint` GitHub Actions check, which lints only the commits in the current push/PR range — the pre-existing, non-conventional history on `main` is never re-checked). Tagging and the GitHub release are owned exclusively by [release-it](https://github.com/release-it/release-it) (`.release-it.json`), which never bumps the version, commits, or publishes to npm — npm publishing stays in `publish.yml`, triggered by the GitHub release release-it creates.
...
2. **Version PR.** Run `pnpm version` (`changeset version`) to consume the pending changesets, bump `package.json`, and update `CHANGELOG.md`. Open this as its own reviewed PR — version edits always go through review before a release, never as a side effect of releasing. Merge it into `main` after CI passes.
