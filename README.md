...
Custom transcript render props own their content semantics and styling. The built-in typed transcript supplies author labels, a centered layout, scroll-driven depth, and a log region. It does not auto-scroll when new content arrives, preserving the reader's position. Composer render props receive stage controls, not a fabricated message API.
